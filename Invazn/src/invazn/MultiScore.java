...
public class MultiScore extends JFrame implements ActionListener
{
  //Window Variables
  ...
  {
    //Initialize labels
    String[] names = IO.getNames(IO.multiPath);
    int[] scores = IO.getScores(IO.multiPath);
    ...
    //Set Window Stuff
    this.setTitle("Multi High Scores");
    this.pack();
  ...
  {
    IO.copyFile(IO.multiPath, IO.defaultPath);
    String[] names = IO.getNames(IO.multiPath);
    int[] scores = IO.getScores(IO.multiPath);
    for (int i = 1; i < nameLabels.length; i++)
    {
